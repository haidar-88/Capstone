...
Opens SUMO-GUI, warms up until ≥150 vehicles exist, discovers 17 clusters of 6 vehicles,
runs the full MVCCP protocol stack over 20 simulated minutes (06:00–06:20, 60x speed).
...
| `simulation/` | SUMO/TraCI integration — orchestrator (`traci_runner.py`), network adapter (`sim_network.py`), TraCI vehicle (`traci_vehicle.py`) |
| `TAPASCologne-0.32.0/` | Cologne traffic dataset — **do not modify** |
| `docs/` | Architecture diagrams and protocol definition |
...
## Key Constants (simulation/traci_runner.py)

| Constant | Value | Notes |
|----------|-------|-------|
| `_DSRC_RANGE_M` | 100.0 m | Radio range for neighbor discovery |
| `_MAX_VEHICLES` | 6 | Max vehicles per platoon |
| `_NUM_CLUSTERS` | 17 | Target cluster count |
| `_BATTERY_CAPACITY_KWH` | 80.0 kWh | Per vehicle |
| `_TIME_SCALE` | 60.0 | Simulation speedup factor |
