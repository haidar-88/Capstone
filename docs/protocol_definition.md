...
## 2. Scope
MVCCP defines:
- Message formats (HELLO, PA, JOIN_OFFER, JOIN_ACCEPT, NACK, ACK, ACKACK, PLATOON_BEACON, PLATOON_STATUS, AIM)
...
1. DISCOVER: listen for PAs and optional AIM.
...
4. WAIT_ACCEPT: start timer and wait for JOIN_ACCEPT or NACK.
5. ACK: upon ACCEPT, send ACK. If NACK, return to EVALUATE.
...
1. AIM_ANNOUNCE: broadcast AIM periodically with capacity info.
2. WAIT_OFFERS: listen for JOIN_OFFER messages during an offer window.
3. EVALUATE_QUEUE: select next consumer from queue (FIFO policy) or refuse with NACK.
4. SEND_ACCEPT: send JOIN_ACCEPT to chosen consumer or NACK if refusing.
...
4. SEND_ACCEPT: send JOIN_ACCEPT to accepted members with position assignment, or NACK if capacity/alignment is insufficient.
...
HELLO -> PA(provider_type = RREH) plus optional AIM ->
Consumer selects RREH -> JOIN_OFFER -> JOIN_ACCEPT (or NACK) -> ACK -> ACKACK ->
...
## 11. Error Handling
- If JOIN_ACCEPT or NACK is not received before timeout, the consumer removes the provider from its local ProviderTable and re-enters EVALUATE.
- If NACK is received, the consumer immediately re-enters EVALUATE.
...
- If an RREH transitions to congested or offline, new PA or AIM messages are broadcast to update queue_time_estimate_s and operational_state.
...
Nodes must validate signatures before acting on PA, JOIN_ACCEPT, PLATOON_BEACON, and AIM messages.
